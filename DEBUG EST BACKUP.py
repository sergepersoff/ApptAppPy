...
# Calculate the Estimated_Revenue for special codes
for code, price_per_unit in special_codes.items():
    # Apply the calculation only for the specified code with zero payment amount
    condition = (df['Transaction_Code'] == code) & (df['Total_Payments_Amount'] == 0)
    df.loc[condition, 'Estimated_Revenue'] = df.loc[condition, 'Units of Service'] * price_per_unit


# Calculate the Estimated_Revenue for special codes, considering underpayment
for code, price_per_unit in special_codes.items():
    ...
    df.loc[condition, 'Expected_Payment'] = df.loc[condition, 'Units of Service'] * price_per_unit
...
df.drop(columns=['Expected_Payment', 'Revenue_Owed'], inplace=True, errors='ignore')
